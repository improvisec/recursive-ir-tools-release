...
The tool would then flag each day having met a certain criteria as "dodgy" and hence flag that user or service pricipal as dodgy. For example, if there is at least one day when the user logged in successfully from at least 4 different countries, that coudl be considered as dodgy. Likewise, having logged in from more than three Internet Service Providers or ASN at any given day looks dodgy. Similarly, registering a new MFA device would be considered as dodgy too if it happens quite recently.
